...
func initConsumer() (*consumergroup.ConsumerGroup, error) {
	...
	cg, err := consumergroup.JoinConsumerGroup(os.Getenv("CONSUMER_GROUP"), []string{os.Getenv("KAFKA_TOPIC")}, []string{os.Getenv("ZOOKEEPER_HOST")}, config)
	if err != nil {
		return nil, err
	}
...
func consume(cg *consumergroup.ConsumerGroup) {
	for {
		...
		case msg := <-cg.Messages():
			...
			if msg.Topic != os.Getenv("KAFKA_TOPIC") {
				continue
			}
